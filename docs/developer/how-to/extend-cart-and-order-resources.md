---
id: extend-cart-and-order-resources
title: Extend cart and order resources
sidebar_label: Extend cart and order resources
---
...
Elastic Path Commerce Cloud does not provide any shipping capabilities by default. This example explains a simple use case with couple of shipping options that are valid across all scenarios.
...
- An Elastic Path Commerce Cloud account and the Client ID and Client Secret for your store, available from [Commerce Manager](../../dashboard/index.md).
...
1. Create entries for the `Shipping Provider` flow fields that you created in the previous step as in the following example:
    ...
    ```sh
    curl -X POST https://api.moltin.com/v2/flows/shipping_provider/entries \
         -H "Authorization: XXXX" \
         -H "Content-Type: application/json" \
         -d $'{
            "data": {
...
1. Do the following and create an order:
    ...
    1. Replace the :cart-reference with a value, such as `1234` in the following request and get the cart:
        ...
        ```sh
        curl -X GET https://api.moltin.com/v2/carts/:cart-reference \
    ...
    1. Add the shipping provider to the cart by creating a relationship entry for the `Ralphio 720` as in the following example:
        ...
        ```sh
        curl -X POST https://api.moltin.com/v2/carts/entries/:cart-reference/relationships/shipping-provider \
             -H "Authorization: Bearer XXXX" \
             -H "Content-Type: application/json" \
             -d $'{
              "data": {
    ...
    1. To Checkout the cart as an anonymous user, use the customer object and make the following request:
        ...
        ```sh
        curl -X POST https://api.moltin.com/v2/carts/:cart-reference/checkout \
             -H "Authorization: Bearer XXXX" \
             -H "Content-Type: application/json" \
             -d $'{
               "data": {
    ...
    1. To add `Ralphio 720` to the order, make the following request and create a relationship entry for the  shipping provider:
        ...
        ```sh
        curl -X POST https://api.moltin.com/v2/orders/:order-id/relationships/shipping-provider \
             -H "Authorization: Bearer XXXX" \
             -H "Content-Type: application/json" \
             -d $'{
              "data": {
    ...
    1. Make the following request and update the order with the consignment ID:
        ...
        ```sh
        curl -X PUT https://api.moltin.com/v2/orders/:order-id \
            -H "Authorization: Bearer XXXX" \
            -H "Content-Type: application/json" \
            -d $'{
              "data": {
...
1. Make the following request and get the order with the shipping details:
    ...
    ```sh
    curl -X GET https://api.moltin.com/v2/orders/:order-id \
          -H "Authorization: Bearer XXXX"
    ```
