---
id: create-custom-carts
title: Create custom carts
sidebar_label: Create custom carts
---

Custom carts allow you to process an order for any type of item you want to sell. The item does not need to exist in Elastic Path Commerce Cloud, which means custom cart items can be used for all sort of use cases.

Places in the user flow you would use custom cart:

- To store a product that does not live in Commerce Cloud for a user to buy
...
- The [playground](https://moltin-cart-example.herokuapp.com/index.html)
- The [code](https://gist.github.com/gje4)


## Scenario 1: Allow a user to buy a product thatʼs not in Commerce Cloud

Setup: A made-up product that does not exist in Commerce Cloud. This means it does not have a `product_id`.
