...
sidebar_label: Create tax rates
---
...
- Tax calculation logic. The `includes_tax` field on the product can be set to true or false:
    - `true` Commerce Cloud performs a backward calculation of taxes meaning the original product price is not affected during checkout.
    - `false` Commerce Cloud calculates tax and add it to the original product price.
