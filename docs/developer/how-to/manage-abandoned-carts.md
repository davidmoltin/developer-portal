---
id: manage-abandoned-carts
title: Manage abandoned carts
sidebar_label: Manage abandoned carts
---

There are three aspects of how Elastic Path Commerce Cloud carts operate:
...
You know this is a new cart if there is no Commerce Cloud cart already associated with the user of your application. In this case, you make the following `POST` call to `v2/flows/abandoned-carts/entries`.
...
You can add two calls from your application to Commerce Cloud after the user has successfully checked out:
...
If the user has not checked out after seven days, the cart session is automatically deleted by Commerce Cloud. We want to make sure that our list is kept in line with that fact. Therefore, you can write a short script which runs once a day.
